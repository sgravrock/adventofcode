...
fn decompress(input: &str) -> String {
	...
	while !scanner.is_empty() {
		let s = match next_repeatable(&mut scanner) {
			Some(rep) => rep,
			None => scanner.scan_to('(').unwrap()
		};
...
fn next_repeatable(scanner: &mut Scanner) -> Option<String> {
	match scan_repetition(scanner) {
		Some(rep) => {
			let mut result = String::new();
	
			for _ in 0..rep.times {
				result.push_str(&rep.s);
			}
