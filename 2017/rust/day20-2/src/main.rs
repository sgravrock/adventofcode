...
fn main() {
	...
	let result = collide_all(particles);
	println!("{}", result.len());
}
...
fn collide_all(mut particles: Vec<Particle>) -> Vec<Particle> {
	let mut stable_iters = 0;
	particles = collide(particles);

	while stable_iters < 10 {
...
fn collide(particles: Vec<Particle>) -> Vec<Particle> {
	let mut by_pos: HashMap<(i32, i32, i32), Vec<Particle>> = HashMap::new();

	for p in particles {
		let added = if let Some(v) = by_pos.get_mut(&p.position) {
			v.push(p);
			true
		} else {
			false
		...
		if !added {
			by_pos.insert(p.position, vec![p]);
		}
	}
...
fn test_collide_all() {
	...
	let actual = collide_all(particles);
	assert_eq!(1, actual.len());
	assert_eq!(3, actual[0].id);
}
