...
class Grid
	...
	# Out of range get returns nil, but out of range set throws
	def [](coord)
		@cells[coord]
	end

	def []=(coord, value)
		if coord.x < 0 || coord.x >= width || coord.y < 0 || coord.y >= height then
			raise "Out of range: #{coord}"
		end
...
class Tests < Microtest::Test
	...
	def test_remove_max
		...
		END
		grid = Grid.new(input)
		assert_equal(43, remove_max(grid))
	end
end
