...
implementation
	...
	procedure RunTests;
		...
		begin
			...
			Enumerate(7);
			if contents[1] <> 1 then
				Fail('Mismatch at [1]');
			if contents[2] <> 2 then
				Fail('Mismatch at [1]');
			if contents[3] <> -3 then
				Fail('Mismatch at [1]');
			if contents[4] <> 4 then
				Fail('Mismatch at [1]');
			if contents[5] <> 0 then
				Fail('Mismatch at [1]');
			if contents[6] <> 3 then
				Fail('Mismatch at [1]');
			if contents[7] <> -2 then
				Fail('Mismatch at [1]');
		...
		begin
			...
			if not ((result[1] = 4) and (result[2] = -3) and (result[3] = 2)) then
