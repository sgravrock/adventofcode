...
fun findHighScore(numPlayers: Int, highestMarble: Int, debug: Boolean = false): Int {
    val playerScores = initScores(numPlayers)
    val ring = Ring(0)
    var currentPlayer = 0
    ...
    for (marble in 1..highestMarble) {
        if (marble % 23 == 0) {
            ...
            val s = playerScores[currentPlayer]
            playerScores[currentPlayer] = s!! + marble + ring.removeCurrent()
        } else {
            ring.moveCW()
            ring.append(marble)
        }
...
fun initScores(numPlayers: Int): MutableMap<Int, Int> {
