...
impl Machine {
	...
	fn do_current_instruction(&mut self) {
		...
		match instruction.opcode {
			1 => {
				self.mem[params.dest.unwrap()] =
					params.arg0.unwrap() + params.arg1.unwrap();
				self.ip += 4;
			},
			2 => {
				self.mem[params.dest.unwrap()] =
			...
			3 => {
				// TODO: Propagate errors from self.input.unwrap()
				self.mem[params.dest.unwrap()] = self.input.unwrap();
				self.ip += 2;
			},
			4 => {
			...
			5 => {
				if params.arg0.unwrap() == 0 {
					...
					// TODO: detect & propagate range errors
					self.ip = params.arg1.unwrap() as usize;
				}
			},
			6 => {
				if params.arg0.unwrap() == 0 {
			...
			_ => {
				panic!("Unrecognized opcode {} at self.ip={}",
					self.mem[self.ip], self.ip)
			}
		}
	}

	fn evaluate_params(&self, instruction: &Instruction) -> ParamValues {
		// TODO: detect & propagate range errors in dest
		match instruction.opcode {
			1 | 2 | 7 | 8 => ParamValues {
				arg0: Some(self.lvalue(&instruction, 0)),
				arg1: Some(self.lvalue(&instruction, 1)),
				dest: Some(self.mem[self.ip + 3] as usize),
			},
			3 => ParamValues {
				arg0: None,
				arg1: None,
				dest: Some(self.mem[self.ip + 1] as usize),
			},
			4 => ParamValues {
				arg0: Some(self.lvalue(&instruction, 0)),
				arg1: None,
	...
	fn lvalue(&self, instruction: &Instruction, param_ix: usize) -> i32 {
		// TODO: detect & propagate bounds errors
