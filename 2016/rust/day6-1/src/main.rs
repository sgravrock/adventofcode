...
fn find_msg(src_msgs: &Vec<&str>) -> String {
	...
	cols.iter()
		.map(|col| most_frequent(col).unwrap())
		.collect()
}
...
fn test_find_msg() {
	...
	assert_eq!(find_msg(&input), "easter");
}
...
fn most_frequent(chars: &Vec<char>) -> Option<char> {
...
fn test_most_frequent() {
	let abca = vec!['a', 'b', 'c', 'a'];
	assert_eq!(most_frequent(&abca), Some('a'));;
