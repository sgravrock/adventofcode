...
func mostExpensiveLoss(items: [Item],
                 playerHitPoints: Int,
	...
	let losers = combos.filter { (combo: Set<Item>) -> Bool in
		...
		return playGame(player: &player, boss: &boss) === boss
	}
	let costs = losers.map { (combo: Set<Item>) -> Int in
