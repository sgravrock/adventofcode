...
func minQE(packages: [Int], weight: Int, passengerCount: Int ) -> Int? {
    return autoreleasepool {
        ...
        for passengerCombo in combos {
            let remainder = packages.filter { !passengerCombo.contains($0) }
            if isValid(remainingPackages: remainder, weight: weight, remainingContainers: 3) {
                return quantumEntanglement(passengerCombo)
            }
        }
...
func isValid(remainingPackages: [Int], weight: Int, remainingContainers: Int) -> Bool {
    return autoreleasepool {
        if remainingContainers == 1 {
            return sum(remainingPackages) == weight
        }
        
        for combo in combinations(remainingPackages, withSum: weight) {
            let ok = autoreleasepool(invoking: { () -> Bool in
                let rest = remainingPackages.filter { !combo.contains($0) }
                return isValid(remainingPackages: rest, weight: weight, remainingContainers: remainingContainers - 1)
            })
